 ...
 * @param {ObjectId} parentTransactionId - Reference to parent transaction (for distributed transactions)
 * @param {Object} distributedTransactions - References to child transactions (for income transactions)
 * @param {boolean} isDistribution - Whether this is a distribution transaction
 * @param {boolean} isEditable - Whether this transaction can be edited directly
 * @param {string} savingsType - For Savings transactions: 'deposit' or 'withdrawal'
...
const TransactionSchema = new Schema({
  ...
  recipient: {
    name: {
      type: String,
      required: function() {
        // Only required for non-Income transactions
        return this.transactionType !== 'Income';
      },
      default: function() {
        // For Income transactions, use description as default
        return this.transactionType === 'Income' ? this.description || 'Income Source' : undefined;
      }
    },
    type: {
      type: String,
      ...
      required: function() {
        return this.transactionType !== 'Income' && this.recipient && this.recipient.name;
      },
      default: function() {
        // For Income transactions, use 'merchant' as default
        return this.transactionType === 'Income' ? 'merchant' : undefined;
      }
    },
    details: {
      type: String // Additional details like account number, UPI ID, etc.
    },
    // Store frequently used recipients for auto-suggestions
    frequency: {
      type: Number,
      default: 1
    }
  ...
  // For parent income transactions to store references to distributed transactions
  ...
  // For Savings transactions: deposit or withdrawal
  savingsType: {
    type: String,
    enum: ['deposit', 'withdrawal'],
    required: function() {
      return this.transactionType === 'Savings';
    },
    default: function() {
      return this.transactionType === 'Savings' ? 'deposit' : undefined;
    }
  }
}, {
...
TransactionSchema.pre('validate', function(next) {
  console.log('Transaction pre-validate hook: transactionType =', this.transactionType);

  // Always ensure recipient object exists
  if (!this.recipient) {
    this.recipient = {};
  }

  // For Income transactions, set default recipient values
  if (this.transactionType === 'Income') {
    console.log('Transaction pre-validate hook: Setting recipient for Income transaction');
  ...
  // For non-Income transactions, validate recipient.name
  if (!this.recipient.name) {
    console.log('Transaction pre-validate hook: Missing recipient.name for non-Income transaction');
